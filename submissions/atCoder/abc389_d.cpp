...
void run() {
        ...
        ll ans = 0;
        const ll MX = 2'000'000;
        auto dist = [&](ll x, ll y) { return x * x - x + y * y - y; };
        ll r2 = R * R;
        rep(y, -MX, MX + 1) {
                /*dbg(y);*/
                auto l = minCheck(-MX, MX, [&](ll x) {
                        ll d1 = dist(x, y + 1);
                        ll d2 = dist(x + 1, y);
                        ll d3 = dist(x, y);
                        ll d4 = dist(x + 1, y + 1);

                        return d1 < r2 and d2 < r2 and d3 < r2 and d4 < r2;
                });

                auto r = maxCheck(-MX, MX, [&](ll x) {
                        ll d1 = dist(x, y + 1);
                        ll d2 = dist(x + 1, y);
                        ll d3 = dist(x, y);
                        ll d4 = dist(x + 1, y + 1);
