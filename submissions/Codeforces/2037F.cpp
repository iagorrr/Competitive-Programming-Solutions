...
void run() {
	...
	auto check = 
		[&](int q) {
			...
			sort(all(es));
			int o = 0;
			for (int i = 0; i < len(es);) {
				auto ei = es[i];
				while (!ei.second and i < len(es) and es[i] == ei)
						o++, i++;
	...
	auto ans = minCheck(1ll, (ll)4e9, check);

	cout << ans.value_or(-1) << '\n';
}

/*8<
  AC, sortings, two pointers, sliding window, binary search
>8*/
