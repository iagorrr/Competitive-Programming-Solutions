...
void calc(int s, const vi2d& adj, vll& ws, vll &ds) {
	ws[s] = 1;
...
void __run() {
	...
	vll ways_s(n), ways_t(n);
	vll ds_s(n, -1), ds_t(n, -1);
	calc(s, adj, ways_s, ds_s);
	calc(t, adj, ways_t, ds_t);
	...
	repn(u, n) {
		if (ds_s[u] + ds_t[u] == d) {
			trav(v, adj[u]) {
				if (ds_s[u] == ds_s[v]) {
					ans = sum(ans, mul(ways_s[v], ways_t[u]));
				}
			}
		}
	}
...
/*8<
 * graphs, shortest paths, combinatorics
>8*/
