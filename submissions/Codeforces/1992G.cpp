...
inline ll binom(ll n, ll k) {
  static const int BINMAX = 1'0'000;
...
ll ways(ll i, ll j, ll N) {
	ll lowers = i - 1;
	ll notChoose = j;
	ll chooseLower =  lowers - j;
	ll rangeLower = min(N, i - 1);

	ll greaters = max(0ll, N-i);
	ll chooseGreater = j - chooseLower;

	ll ret = (binom(greaters, chooseGreater) * binom(rangeLower, chooseLower)) % MOD;
