...
void __run() {
        ...
        function<int(int, int, int, int)> solve = [&](int i, int j, bool a,
                                                      bool b) {
                if (i == n) {
                        if (a == b) return 1;
                ...
                int r1 = solve(i + 1, j, a ^ ai, b ^ bi);
                if (r1 == 0) ret = 2;
                if (r1 == 1) ret = max(ret, 1);

                int r2 = solve(i + 1, j, a ^ bi, b ^ ai);
                if (r2 == 0) ret = 2;
                if (r2 == 1) ret = max(ret, 1);
        ...
        int ans = 1;
        for (int i = 20; i >= 0; i--) {
                memo = vector<vector<vector<int>>>(vector<vector<vector<int>>>(
                        n, vector<vector<int>>(2, vector<int>(2, -1))));

                int cur = solve(0, i, 0, 0);
