...
void run() {
	ll n;cin>>n;
	ll k;cin>>k;
	vll xs(n);for(auto&xi:xs)cin>>xi;
	ll mx=*max_element(all(xs));
	ll totSum=accumulate(all(xs),0ll);
	ll ans=1;
	dbg(totSum);
	for(ll h=2;h<=n;h++){
			// minimo de pilhas que preciso ter
			// pra montar elas com altura h
			ll w=max((totSum+h-1)/h,mx);
			// quantos cards preciso comprar
			// pra ter w pilhas de altura h
			ll need=w-(totSum%w);
			dbg(h,need,w);
			if(totSum+k>=h*w&&(!(totSum%w)||need<=k))ans=h;
	}
	cout<<ans<<endl;
}
