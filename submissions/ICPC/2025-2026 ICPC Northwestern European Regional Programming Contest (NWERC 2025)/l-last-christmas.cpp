...
void __run() {
  ...
  vector<string> xs;
  function<bool(str, str)> cmp = [&](const string a, const string b) {
    if (tot[a] > tot[b]) return true;
    if (tot[a] < tot[b]) return false;

    // untie
    repn(i, M) {
      if (untie[a][i] > untie[b][i]) return true;
      if (untie[a][i] < untie[b][i]) return false;
    }
  ...
  // a - b
  function<int(str, str)> cmp2 = [&](const string a, const string b) {
    if (tot[a] > tot[b]) return 1;
    if (tot[a] < tot[b]) return -1;

    // untie
    repn(i, M) {
      if (untie[a][i] > untie[b][i]) return 1;
      if (untie[a][i] < untie[b][i]) return -1;
    }
  ...
  if (len(xs) == 1 or cmp2(xs[0], xs[1]) != 0) {
    cout << xs.front() << endl;
  } else
    cout << "tie" << endl;
}
