...
template<typename T>
vector<Point<T>> circleInter(Point<T> a,Point<T> b,double r1,double r2) {
	...
	return vector<Point<T>>{mid + per, mid - per};
}
...
void run() {
  ld X,Y,R;cin>>X>>Y>>R;
  auto d=hypot(X,Y);
  auto ans=circleInter(Point<ld>(X,Y),Point<ld>((ld)0,(ld)0),R,sqrt(d*d-R*R));
  if(ans[0]>ans[1])swap(ans[0],ans[1]);
  cout<<fixed<<setprecision(6)<<ans[0].x<<' '<<ans[0].y<<endl;
