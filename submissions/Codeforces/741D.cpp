...
struct Sack {
  ...
  void goPairs(int u, bool keep, FA &ADD, FS &SNAP, FD &DEL, FE &EMPTY,
    ...
    if (heavy[u] != -1)
      goPairs(heavy[u], true, ADD, SNAP, DEL, EMPTY, QRY);
    for (int v : g[u])
      if (v != par[u] && v != heavy[u]) {
        for (int i = tin[v]; i < tout[v]; i++)
          QRY(ord[i], u);
...
void __run() {
  ...
  int neg = -2 * n;
  vi mxDepth(1 << a, neg);
  auto add = [&](int u) { chmax(mxDepth[ds[u]], depth[u]); };
  auto snap = [&](int u) { ansNode[u] = curMx; };
  auto qry = [&](int u, int r) {
    auto du = ds[u];
    auto dr = ds[r];
    auto m = du ^ dr;
    chmax(curMx, -2 * depth[r] + depth[u] + mxDepth[m ^ dr]);
    rep(i, a) {
      auto mi = (1 << i);
      auto x = mi ^ m;
      chmax(curMx, -2 * depth[r] + depth[u] + mxDepth[x ^ dr]);
    }
  };
  auto del = [&](int u) { mxDepth[ds[u]] = neg; };
  auto empty = [&]() { curMx = 0; };

  sack.runPairs(add, snap, del, empty, qry);
  dbg(ansNode);
