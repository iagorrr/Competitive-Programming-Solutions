...
TT T maxE(vector<T> &xs) { return *max_element(all(xs)); }
...
void __run() {
        ...
        bool sohTemA = *max_element(as.begin(), prev(as.end())) < 2;

        // poem os duplex
        vll bs(all(as));
        trav(bi, bs) bi /= 2;
        for (auto bi : bs) ans += bi * 2;

        // da pra por o que sobrou no próprio duplex
        vll rs(all(as));
        trav(ri, rs) ri &= 1;
        ...
        // casinho chato quando só tem A !
        if (sohTemA) {
                ll m = min(as.back() / 2, r);
                dbg(as.back(), r, ans, m);
                ans += m;
        } else {
                sort(all(bs));
                trav(bi, bs) bi = max(0ll, bi - 1);
                while (len(bs) and r) {
                        ll canTake = min(r, bs.back());
                        ans += canTake;
                        r -= canTake;
                        bs.pop_back();
                }
        ...
        // posso por no penúltimo do ciclo (acho que esse caso não tem mais)
        // if (r and as.back() / 2 >= 2) ans++;
