...
void __run() {
	...
	set<int> st; repn(i, n) { st.insert(i+1); }

	set<int> u;
	{
	int s2 = s;
	while (len(st) and s2) {
		auto it = prev(st.upper_bound(s2));
		s2 -= *it;
		u.emp(*it);
		st.erase(it);
	}
	...
	while (len(u) != r-l + 1) {
		...
		trav(ui, u) {
			...
			trav(si, st) {
				if (si >= ui) break;
				if (ui - si != si and st.count(ui-si)) {
					a = si;
					break;
				}
			}
		...
		int ui = v;
		u.erase(ui); st.insert(ui);
		u.insert(a2); st.erase(a2);
		u.insert(ui-a2); st.erase(ui-a2);
