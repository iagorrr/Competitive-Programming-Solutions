...
void __run() {
  ...
  trav(xi, xs) {
    ...
    xi &= 1;
    // you can ignore only if there is already a marked one
    if (len(byParity[xi])) {
      if (done[xi] and byParity[xi].back() <= 0)
        continue;
      done[xi] = 1;

      ans -= byParity[xi].back(), byParity[xi].ppb();
    }
  }
...
/*8<
- Sempre quer marcar os maiores
- esse operação de swap me permite colocar qualquer valor em qualquer lugar ?
    - Somente os que estão a uma distância par um do outro
- A paridade então nunca muda
- Pego sempre o maior disponível naquela paridade para ser marcado
>8*/
