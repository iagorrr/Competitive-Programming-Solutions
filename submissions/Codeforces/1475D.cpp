...
void run() {
        ...
        vll psum(len(one));
        partial_sum(all(one), begin(psum));

        dbg(psum, M);
        auto aux =
                minCheck(1, len(one), [&](int x) { return psum[x - 1] >= M; });
        ll ans = LLONG_MAX;
        if (aux) ans = *aux;
        ...
        rep(i, 0, len(two)) {
                ...
                if (M <= 0)
                        ...
                        auto q1 = minCheck(1, len(one), [&](int x) {
                                return psum[x - 1] >= M;
                        });
                        if (q1) chmin(ans, (i + 1) * 2 + *q1);
                }
        }
...
/*8<
 * AC, sortings, greedy, binary search, prefix sums
>8*/
